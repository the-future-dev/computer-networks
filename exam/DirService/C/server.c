...
int main(int argc, char **argv) {
    ...
    for (;;) {
        ...
        if (FD_ISSET(udpfd, &rset)) {
            ...
            len = sizeof(struct sockaddr_in);
            if (recvfrom(udpfd, &req, sizeof(Request), 0, (struct sockaddr *)&cliaddr,
                         &len) < 0)
            {
                perror("recvfrom");
                continue;
            ...
            fd = open(req.nome_file, O_RDONLY);
            if (fd < 0) {
                printf("File inesistente\n");
                ris = -2;
            } else {
                while (leggiLinea(fd, req.parola_cercata) > 0) {
                    if (strstr(linea, req.parola_cercata)) {
                        ris = 0;
                        break;
                    }
                }
