...
int main(int argc, char **argv) {
    ...
    char               read_char, err[128], word_buffer[WORD_LENGTH], * ext;
    char               newDir[LINE_LENGTH], fileNameTemp[LINE_LENGTH], dir[LINE_LENGTH];
    DIR           *dir1, *dir2, *dir3;
    ...
    for (;;) {
        ...
        if (FD_ISSET(listen_sd, &rset)) {
            printf("Ricevuta richiesta TCP: file di testo del direttorio secondo livello\n");
            len = sizeof(cliaddr);
            ...
            if (fork() == 0) {
                ...
                // #1 Leggo la richiesta del client
                                ...
                                if ((dir2 = opendir(newDir)) != NULL) { // dir sec livello
                                    while ((dd2 = readdir(dir2)) != NULL) {
                                        if (strcmp(dd2->d_name, ".") != 0 &&
                                            strcmp(dd2->d_name, "..") != 0)
                                        {
                                            strcpy(fileNameTemp, newDir);

                                            strcat(fileNameTemp, "/");
                                            strcat(fileNameTemp, dd2->d_name);
                                            
                                            if ((dir3 = opendir(fileNameTemp)) == NULL)
                                            { // file of sec livellov
                                                
                                                // #3 name size
                                                int len = (strlen(fileNameTemp) + 1);
                                                write(conn_sd, &len, sizeof(int));

                                                ext = strstr(dd2->d_name, ".txt");
                                                if (ext != NULL && strcmp(ext, ".txt")==0)
                                                {// text file

                                                    //#4 Write name
                                                    printf("Invio nome: %s\n", dd2->d_name);
                                                    if (write(conn_sd, fileNameTemp, (strlen(fileNameTemp) + 1)) < 0)
                                                    {
                                                    ...
                                                    int fd_temp;
                                                    if ((fd_temp = open((fileNameTemp), O_RDONLY)) < 0) {
                                                        printf("\t>Errore apertura file\n");
                                                        risp = '\1';
                                                        write(conn_sd, &risp, sizeof(char));
                                                    } else { // Se è stato aperto correttamente

                                                        // #5 r/w file content
                                                        while ((nread = read(fd_temp, &read_char, sizeof(char))) > 0) {
                                                            write(conn_sd, &read_char, sizeof(char));
                                                        }
                                                    }
                                                    close(fd_temp);
                                                    printf("\n-> close file\n\n");
