    ...
    for(;;)
    {
        ...
        */ 
        if (FD_ISSET(udp_fd, &rset)){
            ...
            //Realizzazione seriale
            //>
            int risultato = 1, found = 0;
            if ((strcmp(nuova_calza.tipo, "Normale") != 0 && strcmp(nuova_calza.tipo, "Celiaco") != 0)
                ||
                (nuova_calza.carbone != 'N' && nuova_calza.carbone != 'S'))
                {
                    risultato = 0;
                    printf("Probema di tipo");
                }
            
            if (risultato)
            {
                for (index = 0; index < MAX_CALZE; index++){
                    if (strcmp(porta_calze[index].identificatore, nuova_calza.identificatore) == 0){
                        risultato = 0;
                        printf("Identificatore già presente");
                    }
                }
            }
            ...
            if (risultato)
            {
                for (index = 0; index < MAX_CALZE && !found; index++){
                    if (strcmp(porta_calze[index].identificatore, "-1") == 0){
                        found = 1;
                        add_calza_to_portacalze(porta_calze, index, nuova_calza.identificatore, nuova_calza.tipo, nuova_calza.carbone,
                            nuova_calza.citta, nuova_calza.via, nuova_calza.messaggio);
                    }
                } 
            }

            risultato = (found && risultato) ? 0 : -1;

            printf("Invio risultato: %d\n", risultato);
            risultato = htonl(risultato);

            if (sendto(udp_fd, &risultato, sizeof(risultato), 0, (struct sockaddr *) & client_address, sizeof(struct sockaddr_in)) < 0){
                perror("Errore durante SENDTO");
                continue;
            }
...
int add_calza_to_portacalze(Calza * porta_calze, int index, char * id, char * tipo, char carbone, char* citta, char* via, char* messaggio) {
    ...
    strcpy(porta_calze[index].identificatore, id);
    strcpy(porta_calze[index].tipo, tipo);
    porta_calze[index].carbone = carbone;
    strcpy(porta_calze[index].citta, citta);
    strcpy(porta_calze[index].via, via);
    strcpy(porta_calze[index].messaggio, messaggio);
