...
public class ServerCongressoImpl extends UnicastRemoteObject implements
  ...
  public static void main(String[] args) {
    ...
    try {
      RegistryRemotoTagServer registryRemotoTag = (RegistryRemotoTagServer) Naming
          .lookup(completeRemoteRegistryName);
      ServerCongressoImpl serverRMI = new ServerCongressoImpl();
      if (registryRemotoTag.aggiungi(serviceName, serverRMI)) {
    	  System.out.println("Server RMI: Servizio \"" + serviceName + "\": registrato");  
      }
      
      if (registryRemotoTag.associaTag(serviceName, "congresso")) {
    	  System.out.println("Server RMI: Servizio \"" + serviceName + "\": aggiunto tag (congresso)");
      }
      if(registryRemotoTag.associaTag(serviceName, "bellaa")) {
    	  System.out.println("Server RMI: Servizio \"" + serviceName + "\": aggiunto tag (bellaa)");
      }
